...
- **The settings UI offers four thinking levels, not the full profile vocabulary** — the model rows' thinking-intensity checkboxes cover minimal/low/medium/high; `xhigh` and `max` remain settings.yaml-only. Toggling any checkbox rewrites the row's `reasoningEfforts` dict from the offered four, so a hand-declared `xhigh`/`max` entry is dropped by the next UI edit of that row.
