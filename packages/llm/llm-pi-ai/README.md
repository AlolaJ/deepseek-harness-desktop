...
- **The settings UI offers the full thinking-level vocabulary** — the model rows' thinking-intensity checkboxes cover minimal/low/medium/high/xhigh/max, matching `THINKING_LEVELS` in `catalog.ts`. `off` is never a checkbox: every declared dict carries it as the valueless "supported, send nothing" entry. Toggling any checkbox rewrites the row's `reasoningEfforts` dict from the offered levels, so a hand-declared entry is preserved by the next UI edit of that row.
