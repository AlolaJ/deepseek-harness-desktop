...
- Update (2026-09-03): the `Stream ended without finish_reason` member of this wording family is no longer produced for the deterministic case — [[2026-09-03-pi-ai-compat-stream-finish-default]] defaults `supportsFinishReason: false` at resolution for catalog-unknown or protocol-repointed `openai-completions` models, so the classifier's match now sees genuine mid-response drops and routes that opted into strictness.
