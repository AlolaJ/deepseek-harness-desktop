...
;
; Official translation distributed with Inno Setup (licensed under the Inno
; Setup License). Bundled here so python/desktop's installer build does not
; depend on the machine's Inno install shipping this file (winget's per-user
; install omits Chinese).
;
...
ComponentSize1=%1 KB
ComponentSize2=%1 MB
ComponentsDiskSpaceGBLabel=当前选择的组件需要至少 [gb] GB 的可用磁盘空间。
ComponentsDiskSpaceMBLabel=当前选择的组件需要至少 [mb] MB 的可用磁盘空间。
